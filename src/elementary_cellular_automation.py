...
'''
Create pattern dict by pattern_number 
'''
def get_patterns_matches(pattern_number):
    #multiple of 2
    values_patterns = {}
    for n in range(7, -1, -1):
        if pattern_number/2**n == 1:
            value = 'x'
            pattern_number = pattern_number%2**n
        else:
            value = '.'
        values_patterns[patterns[n]] = value
    return values_patterns

def cellular_automaton(text, pattern, generations):
    pattern_matches = get_patterns_matches(pattern)
    for g in range(0, generations):
        text = get_string_generation(text, pattern_matches)
    return text
    
'''
make new string by applying pattern to string
'''
def get_string_generation(text, pattern_matches):
    new_string = ''
    n = len(text)
    for i in range(0, n):
        match = text[i-1]+text[i]+text[(i+1)%n]
        new_string = new_string + pattern_matches[match] 
    return new_string
